...
if os.path.exists(cred_file):
    ...
    get_cred = creds_path.readline();
    creds_str = get_cred.split(",")
...
#commenting this out in case we do need it later on #
...
if 'code' in auth_response:
    cred_retry = get_creds()
    username = cred_retry[0]
    password = cred_retry[1]
    count = +1
...
elif auth_response['code'] == 1012:
    count = +1
    # allow retry credentials 5 times before reboot #
    if count <= 5:
        cred_retry = get_creds()
        username = cred_retry[0]
        password = cred_retry[1]
